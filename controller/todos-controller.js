...
// get all
    ...
    getTodosByID(req, res) {
        ...
        pool.getConnection(function(err, connection) {
            if (err) throw err;
            connection.query(`
                SELECT * FROM todos WHERE deleted_at IS NULL AND todos_id = ?`, [activity_id]
            , function (error, results) {
                if(error) throw error;
                res.json({ 
                    success: 'Success', 
    ...
    updateTodos(req, res) {
        ...
        pool.getConnection(function(err, connection) {
            if (err) throw err;
            connection.query(`
                UPDATE todos SET ? WHERE todos_id = ?`, [data, id]
            , function (error, results) {
                if(error) throw error;
                res.json({ 
                    success: 'Success', 
    ...
    deleteTodos(req, res) {
        ...
        pool.getConnection(function(err, connection) {
            if (err) throw err;
            connection.query(`
                UPDATE todos SET ? WHERE todos_id = ?`, [dataUpdate, id]
            , function (error, results) {
                if(error) throw error;
                res.json({ 
                    success: 'Not Found', 
